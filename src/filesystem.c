...
int main(int argc, char *argv[]) {
    ...
    for (int i = 0; i < args.argc; i++) printf("xX:%s\n", args.argv[i]);
    ...
    extended_args[args.argc] = (char *) malloc(strlen(find_mount_point(argc,argv)));
    strcpy(extended_args[args.argc],find_mount_point(argc,argv));
    ...
    // We call find_mount_point because FUSE_ARGS_INIT changes the mount
    // parameter to its full path, and that forces the user to have root
    // privilege to execute the command, and that has negative security
    // consequences
